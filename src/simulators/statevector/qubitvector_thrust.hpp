...
public:
  ...
  virtual uint_t Compress(uint_t pos, uint_t size, cudaStream_t stream=0, int iStream=0) = 0;
  virtual void Decompress(uint_t pos, uint_t size, cudaStream_t stream=0, int iStream=0) = 0;
...
//    std::cout << "Buffer size: " << m_Buffer.size() << std::endl;
  ...
  // Data compression
  uint_t Compress(uint_t pos, uint_t size, cudaStream_t stream=0, int iStream=0);
  void Decompress(uint_t pos, uint_t size, cudaStream_t stream=0, int iStream=0);
...
{
protected:
  // host_vector does not work for async
  // ref: https://stackoverflow.com/questions/25064383/how-to-asynchronously-copy-memory-from-the-host-to-the-device-using-thrust-and-c
...
template <typename data_t>
uint_t QubitVectorHostBuffer<data_t>::Compress(uint_t pos, uint_t size, cudaStream_t stream, int iStream)
{
  return 0;
}
...
int QubitVectorChunkContainer<data_t>::PutCompressed(QubitVectorChunkContainer &chunks, uint_t dest,
                                                     int chunkBits, cudaStream_t stream, int iStream)
{
...
//    cudaMemcpyAsync(chunks.m_pOff[iStream]->BufferPtr(), m_pOff[iStream]->BufferPtr(), BLOCKS*WARPS_BLOCK*sizeof(int),
//                    cudaMemcpyDeviceToHost, stream);

//    for (int i = 0; i < 504; i++) {
//      std::cout << m_pOff->Get(i) << std::endl;
//    }
    cudaMemcpy(chunks.m_pOff[iStream]->BufferPtr(), m_pOff[iStream]->BufferPtr(), BLOCKS*WARPS_BLOCK*sizeof(int), cudaMemcpyDeviceToHost);
//    for (int i = 0; i < 504; i++) {
...
//      std::cout << "start: " << start << std::endl;
      int off = chunks.m_pOff[iStream]->Get(i);
//      std::cout << "off: " << off << std::endl;
      chunks.m_pOff[iStream]->Set(i, off-((start+1)/2*17));
      memcpy(reinterpret_cast<int*>(chunks.m_pChunks->BufferPtr()+destPos_off)+i,
             chunks.m_pOff[iStream]->BufferPtr()+i, sizeof(int));
    }
                                           ...
                                           int chunkBits,int nChunks,cudaStream_t stream)
{
  ...
  if(m_iDevice >=0 && chunks.DeviceID() >= 0){
    if(m_p2pEnable[chunks.DeviceID()]){
      m_pChunks->Copy(destPos,chunks.m_pChunks,srcPos,size,1,stream);
    }
    else{
      QubitVectorHostBuffer<thrust::complex<data_t>> tmp(size);
      tmp.Copy(0,chunks.m_pChunks,srcPos,size,1);   //D to H
      m_pChunks->Copy(destPos,&tmp,0,size,0,stream);     //H to D
    }
  }
  else{
                                           ...
                                           int chunkBits,int nChunks,cudaStream_t stream)
{
  ...
  if(m_iDevice >=0 && chunks.DeviceID() >= 0){
    if(m_p2pEnable[chunks.DeviceID()]){
      chunks.m_pChunks->Copy(destPos,m_pChunks,srcPos,size,1,stream);
    }
    else{
      QubitVectorHostBuffer<thrust::complex<data_t>> tmp(size);
      tmp.Copy(0,m_pChunks,srcPos,size,1);      //D to H
      chunks.m_pChunks->Copy(destPos,&tmp,0,size,0,stream);  //H to D
    }
  }
  else{
    chunks.m_pChunks->Copy(destPos,m_pChunks,srcPos,size,(DeviceID() >= 0),stream);
  }
                                               ...
                                               uint_t localMask, bool omp_parallel, cudaStream_t stream)
{
  ...
  if(m_iDevice >= 0){
    ...
    cudaSetDevice(m_iDevice);
#endif
    thrust::for_each_n(thrust::cuda::par.on(stream), chunkIter, size, func);
  }
  else{
    if (omp_parallel) {
                                                     ...
                                                     uint_t localMask, bool omp_parallel, cudaStream_t stream)
{
  ...
  if(m_iDevice >= 0){
    ...
    cudaSetDevice(m_iDevice);
#endif
    ret = thrust::transform_reduce(thrust::cuda::par.on(stream), chunkIter, chunkIter + size, func,0.0,thrust::plus<double>());
  }
  else{
    if (omp_parallel) {
...
protected:
  ...
  // Streams
  //-----------------------------------------------------------------------
  cudaStream_t m_Streams[2*AER_NUM_STREAM];
...
//      int num_streams = streams_size();                           // number of streams
      int nGPUBufferPerStream = nGPUBuffer / AER_NUM_STREAM;         // number of streams
      int num_exe = 0;

      // data for compression
      int doubles = 2 * (1ull << chunkBits);

      uchar* dbuf = m_Chunks[iPlace].GetDbuf();;
      uint_t per_dbuf = (doubles+1)/2*17;
      int* cut = m_Chunks[iPlace].GetCut();;
      int* off = m_Chunks[iPlace].GetOff();;
      uint_t per_off = BLOCKS*WARPS_BLOCK;
...
//                                               chunkBits, m_Streams[iStream], iStream);
              ...
              // Switch stream
              iStream = (iStream + 1) % AER_NUM_STREAM; // current stream
            }
          }
        }
